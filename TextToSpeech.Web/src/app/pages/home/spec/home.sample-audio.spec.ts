...
describe('SampleAudioPlayer', () => {
  ...
  interface AudioStub { src: string; play: jasmine.Spy; pause: jasmine.Spy; load: jasmine.Spy }
  let revokeCalls = 0;
  ...
  beforeEach(() => {
    originalAudio = (window as unknown as { Audio?: unknown }).Audio;
    (window as unknown as { Audio: unknown }).Audio = FakeAudio as unknown;
    revokeCalls = 0;
    createdUrls = [];
    ...
    spyOn(URL, 'revokeObjectURL').and.callFake(() => { revokeCalls += 1; });
  });

  afterEach(() => {
    (window as unknown as { Audio?: unknown }).Audio = originalAudio as unknown;
  });
  ...
  it('stop pauses, clears source, loads, and revokes URL', () => {
    ...
    FakeAudio.lastInstance!.play.calls.reset();
  });
