...
describe('SignalRService', () => {
  ...
  it('registers AudioStatusUpdated listener and normalizes progress', () => {
    ...
    const onSpy = jasmine
      .createSpy('on')
      .and.callFake((evt: string, cb: AudioStatusCallback) => {
    ...
    const calls: { id: string; status: string; progress: number | null; error?: string }[] = [];
    service.addAudioStatusListener((id, st, pr, err) => calls.push({ id, status: st, progress: pr, error: err }));

    expect(onSpy).toHaveBeenCalledWith('AudioStatusUpdated', jasmine.any(Function));
  ...
  it('stops and clears hub on stopConnection()', async () => {
    ...
    expect((service as unknown as { hub?: signalR.HubConnection }).hub).toBeUndefined();
  });
  ...
  it('supports multiple listeners', () => {
    ...
    const firstListener = jasmine.createSpy('firstListener');
    const secondListener = jasmine.createSpy('secondListener');
